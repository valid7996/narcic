    ...
    async fetch(request, env, ctx) {
        try {
            ...
            const routes = {
                ...
                testIp: `/${encodeURI(sysConfig.apiRoute)}/api/test-clean-ip`,
            };
            ...
            const isTestIpRoute = reqPath === routes.testIp || reqPath.endsWith('/api/test-clean-ip');
            const isAuthorizedRoute = reqPath === routes.data || reqPath === routes.dash || reqPath === routes.auth || reqPath === routes.sync || reqPath === routes.tg || reqPath === routes.syncPanel || reqPath === routes.logs || isSyncRoute || isUsersRoute || isStatsRoute || isUpdateRoute || isApiKeysRoute || isTestIpRoute;
            ...
            if (!isTelemetryStream) {
                ...
                if (isTestIpRoute) {
                    return await handleTestCleanIp(request, env);
                }
...
async function handleTestCleanIp(request, env) {
    try {
        if (request.method !== "POST") return new Response("405", { status: 405 });
        const data = await request.json();
        ...
        const ipCheck = /^(\d{1,3}\.){3}\d{1,3}$/;
        if (!ipCheck.test(ip)) {
            return new Response(JSON.stringify({ success: false, error: "Invalid IP" }), { headers: { "Content-Type": "application/json" } });
        }
        const sni = (data.sni || "").trim() || new URL(request.url).hostname;
        ...
        const testPromise = (async () => {
            ...
            try {
                ...
                tlsSocket = socket.startTls({ expectedServerHostname: sni });

                const writer = tlsSocket.writable.getWriter();
                const encoder = new TextEncoder();
                const reqStr = "GET /cdn-cgi/trace HTTP/1.1\r\nHost: " + sni + "\r\nConnection: close\r\nUser-Agent: NarcicIPTester/1.0\r\n\r\n";
                ...
                if (/^HTTP\/1\.[01]\s+\d{3}/.test(received)) {
                    return { success: true, ip: ip, ms: Date.now() - start };
                }
                return { success: false, ip: ip, error: "no_valid_response" };
            } catch (e) {
                try { if (tlsSocket) await tlsSocket.close(); else if (socket) await socket.close(); } catch (ce) {}
                return { success: false, ip: ip, error: (e && e.message) || "connection_failed" };
            }
        })();

        const timeoutPromise = new Promise(function (resolve) {
            setTimeout(function () { resolve({ success: false, ip: ip, error: "timeout" }); }, 5000);
        });
      ...
      <script>
          ...
          const i18n = {
              en: {
                  ...
                  lbl_custom_ports_section: "✳️ Custom", ph_custom_port: "e.g. 9443", btn_add_port: "Add", desc_custom_port: "Only ports Cloudflare actually proxies traffic on will work — see developers.cloudflare.com/fundamentals/reference/network-ports.",
              ...
              fa: {
                  title: "دروازه Narcic", pass_ph: "کلید اصلی", login_btn: "ورود به سیستم", err_pass: "دسترسی مسدود شد", missing_db: "⚠️ فضای پایگاه داده یافت نشد! تنظیمات ذخیره نمی‌شوند.",
                  ...
                  lbl_custom_ports_section: "✳️ دلخواه", ph_custom_port: "مثلاً 9443", btn_add_port: "افزودن", desc_custom_port: "فقط پورت‌هایی که Cloudflare واقعاً روی آن‌ها ترافیک را پروکسی می‌کند کار می‌کنند — به مستندات network-ports کلادفلر مراجعه کنید.",
                  ph_clean_ip_name: "نام (اختیاری)", btn_add_ip: "افزودن به مخزن", btn_fetch_clean_ips: "دریافت آی‌پی تمیز (تست از مخزن)",
                  ...
                  changelog_title: "گزارش تغییرات و توضیحات نسخه جدید:",
                   ...
                   ov_gb_unit: "گیگابایت",
                     ...
                     update_pages_unsupported: "نصب یک‌کلیکی از داخل پنل روی Cloudflare Pages پشتیبانی نمی‌شود. تغییرات را در مخزن گیت‌هاب خود push کنید و از طریق git یا wrangler pages deploy دوباره دیپلوی کنید.",
        ...
        async function loadDashboard() {
          ...
          async function testCleanIpEntry(ip, btn) {
              ...
              if (result && result.success) {
                  btn.innerText = result.ms + 'ms'; btn.classList.add('text-emerald-500'); btn.classList.remove('text-red-500');
              } else {
          ...
          function renderCleanIpRepo() {
              ...
              wrap.innerHTML = entries.map(function(e) {
                  ...
                  return '<div class="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-darkborder text-xs">' +
                      ...
                      '<button type="button" onclick="removeCleanIpEntry(\\'' + safeIp + '\\')" class="text-slate-400 hover:text-red-500">✕</button>' +
                      '</div>';
              }).join('');
          }
          ...
          async function testIpLatency(ip) {
              try {
                  const res = await fetch(baseRoute + '/api/test-clean-ip', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ key: sessionKey, ip: ip, sni: window.location.hostname })
                  });
                  const data = await res.json();
                  if (data && data.success) return { ip: ip, ms: data.ms };
                  return null;
              } catch (e) {
                  return null;
              }
          ...
          async function fetchCleanIpsFromRepo() {
              ...
              try {
                  ...
                  if (picked.length === 0) {
                      statusEl.textContent = lang === 'fa' ? 'آی‌پی جدیدی برای افزودن پیدا نشد.' : 'No new IPs found to add.';
                  } else {
                      const entries = parseCleanIpLines();
                      picked.forEach(function(ip) { entries.push({ ip: ip, name: '' }); });
                      writeCleanIpLines(entries);
                      updateUI();
                      statusEl.textContent = (lang === 'fa'
                          ? (picked.length + ' آی‌پی رندوم از مخزن اضافه شد. دوباره بزن تا آی‌پی‌های دیگه بیاد.')
                          : (picked.length + ' random IPs added from the repository. Click again for a different set.'));
                  }
              } catch (e) {
          ...
          function renderCustomPortChip(val, checked) {
              ...
              label.innerHTML = '<input type="checkbox" value="' + val + '" onchange="togglePortCheckbox(\\'' + val + '\\', this.checked)" class="accent-primary" ' + (checked ? 'checked' : '') + '>' +
                  '<span>' + val + '</span>' +
                  '<button type="button" onclick="removeCustomPort(\\'' + val + '\\', event)" class="ms-auto text-slate-400 hover:text-red-500 text-xs">✕</button>';
